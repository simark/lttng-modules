 ...
 *
 * It allocates callstack buffers per-CPU and for each possible interrupt
 * context (4 interrupt nesting: none, softirq, irq, nmi). It therefore uses a
 ...
 * limitation of this approach. We still provide the feature, because it works
 * well for runtime environments having frame pointers.
 *
